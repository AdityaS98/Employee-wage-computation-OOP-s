...
public class employeewage {
	...
	public static int multipleCompanies(String CompanyNAme, int empRatePerHour, int numberOfWorkingDays,int maxHourPerMonth) 
	{
		final int parttime = 1;
		final int fulltime = 2;
		int hours = 0;
		...
		int totalwage = 0;
		int isPresent = 0;
		while (hours <= maxHourPerMonth && totalwage <= numberOfWorkingDays)
			totalwage++;
		int isPresent1 = (int) Math.floor(Math.random() * 10 % 3);
		switch (isPresent1) {
		case parttime:
			hours = 4;
			break;
		...
		int totalempwage = wage * empRatePerHour;
	...
	public static void main(String[] args) {
		...
		monthlywages();
		multipleCompanies("IOCL", 10, 2, 30);
		multipleCompanies("ONGC", 11, 2, 40);
